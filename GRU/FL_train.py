...
# 模型定制化（局部层 + 共享层）
def communication_shared_layers(server_model, models, client_weights, shared_layers=None):
    """
    联邦学习中的通信函数，支持模型定制化（共享层+私有层）

    参数:
    - server_model: 服务器全局模型
    - models: 客户端模型列表
    - client_weights: 客户端权重列表
    - shared_layers: 需要聚合的共享层名称列表，如果为None则聚合所有层（除了BN层）
    """
    client_num = len(models)
    with torch.no_grad():
        for key in server_model.state_dict().keys():
            # 判断是否为BN层
            is_bn_layer = 'bn' in key

            # 判断是否为共享层（如果shared_layers为None，则默认所有层都是共享层，除了BN层）
            is_shared_layer = shared_layers is None or any(layer in key for layer in shared_layers)

            # 只聚合共享层且非BN层的参数
            if is_shared_layer and not is_bn_layer:
...
# ]
def communication_shared_layers_dropout_rate(server_model, models, client_weights, shared_layers=None, dropout_rate=0.2):
    """
    联邦学习中的通信函数，支持模型定制化（共享层+私有层）和参数随机丢弃
    ...
    with torch.no_grad():
        for key in server_model.state_dict().keys():
            ...
            if is_shared_layer and not is_bn_layer:
                ...
                # 随机选择部分客户端参与聚合（类似Dropout）
                active_clients = []
                for client_idx in range(client_num):
                    if torch.rand(1).item() > dropout_rate:  # 随机保留客户端
                        active_clients.append(client_idx)

                # 如果没有客户端被选中，至少保留一个
                if len(active_clients) == 0:
                    active_clients = [torch.randint(0, client_num, (1,)).item()]

                # 重新计算有效客户端的权重
                total_weight = sum([client_weights[i] for i in active_clients])
                normalized_weights = {i: client_weights[i] / total_weight for i in active_clients}

                # 聚合选中的客户端模型参数
                for client_idx in active_clients:
                    temp += normalized_weights[client_idx] * models[client_idx].state_dict()[key]
...
# ]
#
# server_model, client_models = communication_shared_layers_dropout_rate(
#     server_model=server_model,
#     models=client_models,
#     client_weights=client_weights,
#     shared_layers=shared_layers,
#     dropout_rate=0.2  # 每次迭代随机丢弃20%的客户端
# )
...
server_model=GRUModel().to(device)
...
for epoch in range(server_model_epoch):
    ...
    server_volve_loss_acc_mse_mae.to_csv('./FL_output/server/volve/server_volve_loss_acc_mse_mae.csv', sep=",", index=True)
    server_volve_pre_ture_test.to_csv('./FL_output/server/volve/server_volve_pre_ture_test.csv', sep=",", index=True)
    server_xj_loss_acc_mse_mae.to_csv('./FL_output/server/xj/server_xj_loss_acc_mse_mae.csv', sep=",", index=True)
    server_xj_pre_ture_test.to_csv('./FL_output/server/xj/server_xj_pre_ture_test.csv', sep=",", index=True)
    server_bh_loss_acc_mse_mae.to_csv('./FL_output/server/bh/server_bh_loss_acc_mse_mae.csv', sep=",", index=True)
    server_bh_pre_ture_test.to_csv('./FL_output/server/bh/server_bh_pre_ture_test.csv', sep=",", index=True)

    clint0_volve_loss_acc_mse_mae.to_csv('./FL_output/client/volve/clint0_volve_loss_acc_mse_mae.csv', sep=",", index=True)
    clint0_volve_pre_ture_test.to_csv('./FL_output/client/volve/clint0_volve_pre_ture_test.csv', sep=",", index=True)
    clint1_xj_loss_acc_mse_mae.to_csv('./FL_output/client/xj/clint1_xj_loss_acc_mse_mae.csv', sep=",", index=True)
    clint1_xj_pre_ture_test.to_csv('./FL_output/client/xj/clint1_xj_pre_ture_test.csv', sep=",", index=True)
    clint2_bh_loss_acc_mse_mae.to_csv('./FL_output/client/bh/clint2_bh_loss_acc_mse_mae.csv', sep=",", index=True)
    clint2_bh_pre_ture_test.to_csv('./FL_output/client/bh/clint2_bh_pre_ture_test.csv', sep=",", index=True)

    acc_loss_plot_one(server_volve_loss_acc_mse_mae['test_loss'], 'loss','./FL_output/server/volve/server_test_loss.png')
    acc_loss_plot_one(server_volve_loss_acc_mse_mae['test_r2'], 'r2', './FL_output/server/volve/server_test_r2.png')
    acc_loss_plot_one(server_xj_loss_acc_mse_mae['test_loss'], 'loss','./FL_output/server/xj/server_test_loss.png')
    acc_loss_plot_one(server_xj_loss_acc_mse_mae['test_r2'], 'r2', './FL_output/server/xj/server_test_r2.png')
    acc_loss_plot_one(server_bh_loss_acc_mse_mae['test_loss'], 'loss','./FL_output/server/bh/server_test_loss.png')
    acc_loss_plot_one(server_bh_loss_acc_mse_mae['test_r2'], 'r2', './FL_output/server/bh/server_test_r2.png')

    acc_loss_plot_two(clint0_volve_loss_acc_mse_mae['train_r2'], clint0_volve_loss_acc_mse_mae['test_r2'], 'r2','./FL_output/client/volve/clint0_volve_r2.png')
    acc_loss_plot_two(clint0_volve_loss_acc_mse_mae['train_loss'], clint0_volve_loss_acc_mse_mae['test_loss'], 'r2','./FL_output/client/volve/clint0_volve_loss.png')
    acc_loss_plot_two(clint1_xj_loss_acc_mse_mae['train_r2'], clint1_xj_loss_acc_mse_mae['test_r2'], 'r2','./FL_output/client/xj/clint1_xj_r2.png')
    acc_loss_plot_two(clint1_xj_loss_acc_mse_mae['train_loss'], clint1_xj_loss_acc_mse_mae['test_loss'], 'r2','./FL_output/client/xj/clint1_xj_loss.png')
    acc_loss_plot_two(clint2_bh_loss_acc_mse_mae['train_r2'], clint2_bh_loss_acc_mse_mae['test_r2'], 'r2','./FL_output/client/bh/clint2_bh_r2.png')
    acc_loss_plot_two(clint2_bh_loss_acc_mse_mae['train_loss'], clint2_bh_loss_acc_mse_mae['test_loss'], 'r2','./FL_output/client/bh/clint2_bh_loss.png')

    true_test_plot(server_volve_pre_ture_test['depth'], server_volve_pre_ture_test['true'], server_volve_pre_ture_test['pre'], 'test',
                   './FL_output/server/volve/server_volve_pre_ture_test.png')
    true_test_plot(server_xj_pre_ture_test['depth'], server_xj_pre_ture_test['true'], server_xj_pre_ture_test['pre'], 'test',
                   './FL_output/server/xj/server_xj_pre_ture_test.png')
    true_test_plot(server_bh_pre_ture_test['depth'], server_bh_pre_ture_test['true'], server_bh_pre_ture_test['pre'], 'test',
                   './FL_output/server/bh/server_bh_pre_ture_test.png')
    true_test_plot(clint0_volve_pre_ture_test['depth'], clint0_volve_pre_ture_test['true'], clint0_volve_pre_ture_test['pre'], 'test',
                   './FL_output/client/volve/clint0_volve_pre_ture_test.png')
    true_test_plot(clint1_xj_pre_ture_test['depth'], clint1_xj_pre_ture_test['true'], clint1_xj_pre_ture_test['pre'], 'test',
                   './FL_output/client/xj/clint1_xj_pre_ture_test.png')
    true_test_plot(clint2_bh_pre_ture_test['depth'], clint2_bh_pre_ture_test['true'], clint2_bh_pre_ture_test['pre'], 'test',
                   './FL_output/client/bh/clint2_bh_pre_ture_test.png')

    torch.save(models[0].state_dict(), './FL_output/model/model_0_volve.pkl')
    torch.save(models[1].state_dict(), './FL_output/model/model_1_xj.pkl')
    torch.save(models[2].state_dict(), './FL_output/model/model_2_bh.pkl')
    print('save clint model')
    torch.save(server_model.state_dict(), './FL_output/model/server_model.pkl')
