...
def initiate():
    ...
    for epoch in range(1000):
        ...
        loss_acc_mse_mae.to_csv('./output/bh/out/loss_acc_mse_mae_.csv', sep=",", index=True)

        torch.save(model.state_dict(), './output/bh/model/Model_bh.pkl')

        csv_train.to_csv('./output/bh/out/rel_pre_train.csv', sep=",", index=True)
        csv_test.to_csv('./output/bh/out/rel_pre_test.csv', sep=",", index=True)

        acc_loss_plot_two(loss_acc_mse_mae['train_loss'], loss_acc_mse_mae['test_loss'], 'loss',
                      './output/bh/out/loss.png')
        acc_loss_plot_two(loss_acc_mse_mae['train_r2'], loss_acc_mse_mae['test_r2'], 'r2',
                      './output/bh/out/acc.png')
        true_test_plot(csv_test['test_depth'], csv_test['test_true'], csv_test['test_pre'], 'test',
                       './output/bh/out/pre_true_test.png')
        true_test_plot(csv_train['train_depth'], csv_train['train_true'], csv_train['train_pre'], 'train',
                        './output/bh/out/pre_true_train.png')
