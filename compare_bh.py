...
from utility_compare_bh import *
...
bh_train,bh_train_y_max,bh_train_y_min, bh_train_de_max,bh_train_de_min = data_load(bh_15)
...
def initiate():
    ...
    for epoch in range(100):
        start = datetime.now()
        model.train()
        train(model, bh_train,optimizer)
        model.eval()
        ...
        loss_acc_mse_mae.to_csv('./output0518/compare/bh1/out/loss_acc_mse_mae_.csv', sep=",", index=True)

        torch.save(model.state_dict(), './output0518/compare/bh1/model/Model_bh.pkl')

        csv_train.to_csv('./output0518/compare/bh1/out/rel_pre_train.csv', sep=",", index=True)
        csv_test.to_csv('./output0518/compare/bh1/out/rel_pre_test.csv', sep=",", index=True)

        acc_loss_plot_two(loss_acc_mse_mae['train_loss'], loss_acc_mse_mae['test_loss'], 'loss',
                      './output0518/compare/bh1/out/loss.png')
        acc_loss_plot_two(loss_acc_mse_mae['train_r2'], loss_acc_mse_mae['test_r2'], 'r2',
                      './output0518/compare/bh1/out/acc.png')
        true_test_plot(csv_test['test_depth'], csv_test['test_true'], csv_test['test_pre'], 'test',
                       './output0518/compare/bh1/out/pre_true_test.png')
        true_test_plot(csv_train['train_depth'], csv_train['train_true'], csv_train['train_pre'], 'train',
                        './output0518/compare/bh1/out/pre_true_train.png')
