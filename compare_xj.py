...
from utility_compare_xj import *
...
xj_train,train_y_max,train_y_min, train_de_max,train_de_min = data_load(xj_3)
xj_test,test_y_max,test_y_min, test_de_max,test_de_min =  data_load(xj_1)
...
def initiate():
    ...
    for epoch in range(100):
        ...
        test_acc, test_r2, test_mse, test_mae, test_loss, true_test, pre_test, test_depth = test(model, xj_test,
        ...
        loss_acc_mse_mae.to_csv('./output0518/compare/xj1/out/loss_acc_mse_mae_.csv', sep=",", index=True)

        torch.save(model.state_dict(), './output0518/compare/xj1/model/Model_xj.pkl')

        csv_train.to_csv('./output0518/compare/xj1/out/rel_pre_train.csv', sep=",", index=True)
        csv_test.to_csv('./output0518/compare/xj1/out/rel_pre_test.csv', sep=",", index=True)

        acc_loss_plot_two(loss_acc_mse_mae['train_loss'], loss_acc_mse_mae['test_loss'], 'loss',
                      './output0518/compare/xj1/out/loss.png')
        acc_loss_plot_two(loss_acc_mse_mae['train_r2'], loss_acc_mse_mae['test_r2'], 'r2',
                      './output0518/compare/xj1/out/acc.png')
        true_test_plot(csv_test['test_depth'], csv_test['test_true'], csv_test['test_pre'], 'test',
                       './output0518/compare/xj1/out/pre_true_test.png')
        true_test_plot(csv_train['train_depth'], csv_train['train_true'], csv_train['train_pre'], 'train',
                        './output0518/compare/xj1/out/pre_true_train.png')
